        ...
        if (DataLen > 0) {
            ...
            if (len == DataLen) {
				...
				if (strcmp(SensorType, "LidarRSI") == 0) {
					if(RSDScfg.ConfigFlag == 0){
						RSDS_LidarRSI_ReadSpec_VehicleInfoFile(RSDScfg.VehicleFile, RSDScfg.MoviePort);
						RSDS_LidarRSI_Init_IntensityCalc();
						RSDS_LidarRSI_Valid();
						
						RSDScfg.ConfigFlag = 1;
						printf ("Lidar: Setting Sensor Configuration Done.\n");
					}
					
					processLidarData_toSensorPacket(data);
...
void processLidarData_toSensorPacket(const char *dataP)
{
    int i_sens, i_det;
    ...
    for (i_sens = 0; i_sens < nSensors; i_sens++) {
		...
		RSDS_LidarRSI_Config_Out(DataBlockLidar);
...
RSDS_Init(void)
{
	...
	RSDScfg.ConfigFlag = 0;
	RSDScfg.VehicleFile = "Examples/DemoCar";
...
main (int argc, char *const*argv)
{
    ...
    while (*++argv != NULL) {
		...
		} else if (strcmp(*argv, "-f") == 0 && argv[1]) {
			RSDScfg.VehicleFile = *++argv;
			printf("VehicleFile : %s\n", RSDScfg.VehicleFile);
	...
	//LidarRSI_Model free Memory
	RSDS_LidarRSI_Config_Cleanup();
